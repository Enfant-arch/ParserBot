...
def position_open_edit_ap(remover, category_id):
    ...
    for a in range(remover, len(get_positions)):
        if x < count_page:
            if get_positions[a][3] == 2:
                get_items = get_itemsx("*", position_id=get_positions[a][1])
                keyboard.add(InlineKeyboardButton(f"{get_positions[a][2]} | {get_positions[a][5]}руб",
                                              callback_data=f"position_edit:{get_positions[a][1]}:{remover}:{category_id}"))
            else:
                get_items = get_itemsx("*", position_id=get_positions[a][1])
                keyboard.add(InlineKeyboardButton(f"{get_positions[a][2]} | {get_positions[a][4]}руб | {len(get_items)}шт",
                                              callback_data=f"position_edit:{get_positions[a][1]}:{remover}:{category_id}"))
        x += 1
...
def buy_item_item_position_ap(remover, category_id):
    ...
    for a in range(remover, len(get_positions)):
        if x < count_page:
            get_items = get_itemsx("*", position_id=get_positions[a][1])
            if get_positions[a][3] == 2:
                keyboard.add(InlineKeyboardButton(f"{get_positions[a][2]} | {get_positions[a][5]} RUB |",
