...
from keyboards.default import items_default, skip_send_image_default, cancel_send_image_default, \
    finish_load_items_default, type_items_default
...
async def position_select_category_for_create(call: CallbackQuery, state: FSMContext):
    ...
    await StoragePosition.here_input_type_position.set()
    await bot.delete_message(call.from_user.id, call.message.message_id)
    await bot.send_message(call.from_user.id,
                           "<b>📁 Введите тип позиции 🏷</b>", reply_markup=type_items_default)

# Выбор типа  для создания позиции
@dp.message_handler(IsAdmin(),  state=StoragePosition.here_input_type_position)
async def position_select_category_for_create(call: types.Message, state: FSMContext):
    category_type = call.text
    c_type = 0
    match category_type:
        case "📜 LOG:PASS ➕": c_type = 1
        case "🖥 Лицензии  ➕": c_type = 2
        case "📦 Лог ➕": c_type = 3


    async with state.proxy() as data:
        data["here_input_type"] = c_type
    await StoragePosition.here_input_position_name.set()
    await bot.delete_message(call.from_user.id, call.message_id - 1 )
    await bot.send_message(call.from_user.id,
                           "<b>📁 Введите название для позиции 🏷</b>", reply_markup=types.ReplyKeyboardRemove())
...
async def position_input_name(message: types.Message, state: FSMContext):
    ...
    if data["here_input_type"] == 2:
                await message.answer("<b>⚠️⚠️⚠️ВНИАМАНИЕ ваш товар имеет тип лицензионного</b>\n❕Просьба указать цену за день подписки 💰")
                await StoragePosition.here_input_position_price_per_day.set()
    else:
...
# Принятие цены для лицензии \ 1 день
@dp.message_handler(IsAdmin(), state=StoragePosition.here_input_position_price_per_day)
async def position_input_price_per_day(message: types.Message, state: FSMContext):
    if message.text.isdigit():
        if int(message.text) >= 0:
            async with state.proxy() as data:
                data["here_input_position_price_per_day"] = message.text 
            await StoragePosition.here_input_position_price_per_week.set()
            await message.answer("❕Просьба указать цену за неделю подписки 💰" )
        else:
            await StoragePosition.here_input_position_price_per_day.set()
            await message.answer("<b>❌ Минимальная цена для позиции 0 рублей.</b>\n"
                                 "📁 Введите цену для позиции 💰")
...
# Принятие цены для лицензии \ 7 дней
@dp.message_handler(IsAdmin(), state=StoragePosition.here_input_position_price_per_week)
async def position_input_price_per_week(message: types.Message, state: FSMContext):
    if message.text.isdigit():
        if int(message.text) >= 0:
            async with state.proxy() as data:
                data["here_input_position_price_per_week"] = message.text 
            await StoragePosition.here_input_position_price_per_month.set()
            await message.answer("❕Просьба указать цену за месяц подписки 💰")
        else:
            await StoragePosition.here_input_position_price_per_week.set()
            await message.answer("<b>❌ Минимальная цена для позиции 0 рублей.</b>\n"
                                 "📁 Введите цену для позиции 💰")
...
# Принятие цены для лицензии \ 30 дней
@dp.message_handler(IsAdmin(), state=StoragePosition.here_input_position_price_per_month)
async def position_input_price_per_month(message: types.Message, state: FSMContext):
    if message.text.isdigit():
        if int(message.text) >= 0:
            async with state.proxy() as data:
                data["here_input_position_price_per_month"] = message.text 
            await StoragePosition.here_input_position_basepath.set()
            await message.answer("<b>Введите ссылку для выдачи лицензии :</b>\nДанные для hwid, excexpt_time, type установите как ___\n(3x)")
        else:
            await StoragePosition.here_input_position_price_per_month.set()
            await message.answer("<b>❌ Минимальная цена для позиции 0 рублей.</b>\n"
                                 "📁 Введите цену для позиции 💰")
...
# Принятие ссылки добавление товара
@dp.message_handler(IsAdmin(), state=StoragePosition.here_input_position_basepath)
async def position_input_price_per_month(message: types.Message, state: FSMContext):
    async with state.proxy() as data:
        if (await is_url(message.text)) is not None:
            data["base_path"] = message.text
            print(data["base_path"])
        else:
            await StoragePosition.here_input_position_basepath.set()
            await message.answer("<b>Некорректный url !</b>\nДанные для hwid, excexpt_time, type установите как ___\n(3x)")

    await StoragePosition.here_input_position_changepath.set()
    await message.answer("<b>Введите ссылку для замены лицензии :</b>\nДанные для hwid, type установите как ___\n(3x)")

@dp.message_handler(IsAdmin(), state=StoragePosition.here_input_position_changepath)
async def position_input_price_per_month(message: types.Message, state: FSMContext):
    async with state.proxy() as data:
        data["change_url"] = message.text 
    await StoragePosition.here_input_position_discription.set()
    await message.answer("<b>📁 Введите описание для позиции 📜</b>\n"
                            "❕ Вы можете использовать HTML разметку")
...
async def position_skip_get_image(message: types.Message, state: FSMContext):
    async with state.proxy() as data:
        ...
        position_type = data["here_input_type"]
        ...
        await state.finish()
        if data["here_input_type"] == 2:
            price_per_day = data["here_input_position_price_per_day"]
            price_per_week  = data["here_input_position_price_per_week"]
            price_per_mounth = data["here_input_position_price_per_month"]
            base_path = data["base_url"]
            change_link = data["change_url"]
            add_positionx(position_id=position_id[0], position_name=position_name, position_type=position_type,
                           position_price=0, position_price_day=price_per_day, 
                        position_price_week=price_per_week, position_price_month=price_per_mounth,
                          position_discription = position_discription, position_image="", position_date=get_dates(), category_id=catategory_id, base_path=base_path, path_change=change_link)
            await message.answer("<b>📁 Позиция была успешно создана ✅</b>",
                                reply_markup=items_default)
        else: 
            position_price = data["here_input_position_price"]
            add_positionx(position_id[0], position_name, position_type, position_price, position_discription,
...
async def position_get_image(message: types.Message, state: FSMContext):
    async with state.proxy() as data:
        ...
        if data["here_input_type"] == 2:
            ...
            add_positionx(position_id=position_id[0], position_name=position_name, position_type=position_type, position_price=0, position_price_day=price_per_day,
                        position_price_week=price_per_week, position_price_month=price_per_mounth, position_discription= position_discription,
                    position_image=position_photo, position_date=get_dates(), category_id=catategory_id, base_path=base_path, path_change=change_link)
            await message.answer("<b>📁 Позиция была успешно создана ✅</b>",
                                reply_markup=items_default)
        else: 
            position_price = data["here_input_position_price"]
...
# Принятие данных лизензии
@rate_limit(0)
@dp.message_handler(IsAdmin(), state=StorageItems.here_add_items)
async def input_licence_data(message: types.Message, state: FSMContext):
    delete_msg = await message.answer("<b>⌛ Ждите, товары добавляются...</b>")
    count_add = 0
    get_all_items = message.text.split("\n\n")
    for check_item in get_all_items:
